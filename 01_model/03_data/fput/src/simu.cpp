...
int main() {
    ...
    cout << "=======================================" << endl;
    cout << endl;
    cout << "Start simulation of fput..." << endl;
    cout << endl;
    ...
    try{
        {
            h5fid = H5File("/home/victorw/ownCloud/IRCAM/03_DOCS/38_FPUT_problem_DAE/02_python/01_model/03_data/data/results.h5", H5F_ACC_RDWR);
        }
    }
    catch(FileIException error){
        {
            cerr << "Failed opening /home/victorw/ownCloud/IRCAM/03_DOCS/38_FPUT_problem_DAE/02_python/01_model/03_data/data/results.h5 file" << endl;
            exit(1);
        }
    }
    ...
    for (unsigned int n = 0; n < nt; n++) {
        ...
        for (unsigned int ind=0; ind<15; ind++){
            mystruct.x[ind] = fput.x_vector()[ind];
        }
        for (unsigned int ind=0; ind<15; ind++){
            mystruct.dx[ind] = fput.dx_vector()[ind];
        }
        for (unsigned int ind=0; ind<15; ind++){
            mystruct.dxH[ind] = fput.dxH_vector()[ind];
        }
    ...
    cout << "/home/victorw/ownCloud/IRCAM/03_DOCS/38_FPUT_problem_DAE/02_python/01_model/03_data/data/results.h5"<< endl;
    cout << endl;
